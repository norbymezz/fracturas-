...
with tabs[4]:
    ...
    t_days = np.logspace(-3, 2, 80)
    t_sec  = t_days * DAY_TO_S

    P = np.zeros((len(t_days), len(st.session_state.wells)))
    for i, t in enumerate(t_sec):
        P[i, :] = invert_stehfest_vec(Fvec, t, N=14)
    ...
    figA1 = go.Figure()
    for j in range(len(st.session_state.wells)):
        figA1.add_trace(go.Scatter(x=t_days, y=Pwf[:, j], mode="lines+markers", name=f"Pwf w{j+1}"))
    figA1.update_layout(height=420, title="Presión por pozo",
                        xaxis=dict(title="t [day]", type="log"),
    ...
    if len(st.session_state.wells) >= 2:
        figA2 = go.Figure()
        for j in range(len(st.session_state.wells) - 1):
            dP = Pwf[:, j] - Pwf[:, j+1]
            figA2.add_trace(go.Scatter(x=t_days, y=dP, mode="lines+markers",
                                       name=f"ΔP w{j+1}-w{j+2}"))
        figA2.update_layout(height=320, title="ΔP entre pozos vecinos",
                            xaxis=dict(title="t [day]", type="log"),
    ...
    for wname, sched in schedules.items():
        tvals = [s["t_ini"] for s in sched]; qvals = [s["q"] for s in sched]
        figA3.add_trace(go.Scatter(x=tvals, y=qvals, mode="lines+markers",
                                   line_shape="hv", name=f"{wname}"))
    figA3.update_layout(height=320, title="Schedules de caudal q(t)",
                        xaxis=dict(title="t [day]"), yaxis=dict(title="q [STB/d]"))
    st.plotly_chart(figA3, use_container_width=True)

    figA4 = go.Figure()
    for j in range(len(wells)):
        figA4.add_trace(go.Scatter(x=t_days, y=Pwf[:, j], mode="lines", name=f"Pwf w{j+1}"))
    for wname, sched in schedules.items():
        tvals = [s["t_ini"] for s in sched]; qvals = [s["q"] for s in sched]
        figA4.add_trace(go.Scatter(x=tvals, y=qvals, mode="lines+markers",
                                   name=f"q {wname}", yaxis="y2"))
    figA4.update_layout(height=420, title="Presión y caudal juntos",
                        xaxis=dict(title="t [day]", type="log"),
                        yaxis=dict(title="Pwf [psi]"),
    ...
    Pwf1 = Pwf[:, 0]; dP1 = p0_psi - Pwf1

    figB1 = go.Figure()
    figB1.add_trace(go.Scatter(x=t_days, y=dP1, mode="lines+markers", name="ΔP"))
    figB1.update_layout(height=360, title="ΔP vs t (log-log)",
                        xaxis=dict(title="t [day]", type="log"),
                        yaxis=dict(title="ΔP [psi]", type="log"))
    st.plotly_chart(figB1, use_container_width=True)

    figB2 = go.Figure()
    figB2.add_trace(go.Scatter(x=t_days, y=Pwf1, mode="lines", name="Pwf"))
    figB2.update_layout(height=360, title="Pwf vs t",
                        xaxis=dict(title="t [day]", type="log"),
                        yaxis=dict(title="Pwf [psi]"))
    ...
    if schedules:
        ...
        tvals = [s["t_ini"] for s in schedules[w1]]; qvals = [s["q"] for s in schedules[w1]]
        figB3 = go.Figure()
        figB3.add_trace(go.Scatter(x=tvals, y=qvals, mode="lines+markers", name="Q(t)"))
        figB3.update_layout(height=360, title="Q(t) — Pozo 1",
                            xaxis=dict(title="t [day]"), yaxis=dict(title="Q [STB/d]"))
        st.plotly_chart(figB3, use_container_width=True)

    dp_dlogt = np.gradient(np.log(np.clip(P[:,0], 1e-12, 1e12)), np.log(t_days))
    figB4 = go.Figure()
    figB4.add_trace(go.Scatter(x=t_days, y=P[:,0], mode="lines", name="pD (aprox)"))
    figB4.add_trace(go.Scatter(x=t_days, y=np.abs(dp_dlogt), mode="lines",
                               name="|dpD/d ln t|"))
    figB4.update_layout(height=360, title="pD y derivada (aprox.)",
                        xaxis=dict(title="t [day]", type="log"),
