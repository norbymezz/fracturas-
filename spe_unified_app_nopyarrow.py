...
# spe_unified_app_nopyarrow.py
# Igual que spe_unified_app.py pero SIN depender de pandas/pyarrow.
# Muestra tabla con plotly.graph_objects.Table y ofrece CSV para descargar.
...
st.set_page_config(page_title="SPE Unificada (sin pandas)", layout="wide")
st.title("SPE - Ejemplos Unificados (102834-PA & 125043-PA) - sin pandas")

with st.sidebar:
    st.header("Configuracion")
    model_kind = st.selectbox("Modelo", ["125043-PA · Trilinear (SRV/ORV)","102834-PA · Bloques (heterogeneo)"])
    ...
    st.caption("Evita pandas/pyarrow.")

st.markdown("### Propiedades de fluido y formacion")
...
if run:
    ...
    # --- Tabla sin pandas ---
    data = {"t_day": ts}
    for i in range(nW): data[f"Pwf_{i+1}_psi"] = P[:,i]
    headers = list(data.keys())
    cells = [data[k] for k in headers]
    table_fig = go.Figure(data=[go.Table(header=dict(values=headers),
    ...
    for r in range(len(ts)):
        row = [data[h][r] for h in headers]
        writer.writerow(row)
    st.download_button("Descargar CSV de resultados", data=csv_buffer.getvalue(),
                       file_name="pwf_results.csv", mime="text/csv")
