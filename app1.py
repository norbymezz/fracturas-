...
# ====== Helpers gráficos ======
def apply_log_axis_controls(fig, key_prefix: str, default_x_log: bool = False, default_y_log: bool = False):
    c1, c2 = st.columns(2)
    with c1:
        log_x = st.checkbox("Eje X log", value=default_x_log, key=f"{key_prefix}_logx")
    with c2:
        log_y = st.checkbox("Eje Y log", value=default_y_log, key=f"{key_prefix}_logy")
    fig.update_xaxes(type="log" if log_x else "linear")
    fig.update_yaxes(type="log" if log_y else "linear")
    return fig

# ====== UI base ======
st.set_page_config(page_title="app1.py SPE-215031-PA — Interferencia (desde cero)", layout="wide")
st.title("app1.py SPE-215031-PA — Interferencia de pozos horizontales (desde cero)")
...
with tabs[1]:
    ...
    if xs and ys and all(v > 0 for v in xs) and all(v > 0 for v in ys):
        apply_log_axis_controls(fig, "mapa_planta")
...
with tabs[4]:
    ...
    st.markdown("## Resultados - Diagnóstico Completo")

    # pozos, geometría y F(s) desde sesión
    ...
    # schedules simples derivados de los pozos
    schedules: Dict[str, list] = {}
    for j, w in enumerate(wells):
        if not w.t_days or not w.q_stbd:
            continue
        schedules[f"w{j+1}"] = [
            {"t_ini": float(t), "q": float(q)}
            for t, q in zip(w.t_days, w.q_stbd)
        ]

    # cálculo base: inversión de Laplace para varios tiempos
    t_days = np.logspace(-3, 2, 80)        # días
    t_sec  = t_days * DAY_TO_S

    P = np.zeros((len(t_days), len(wells)))
    for i, t in enumerate(t_sec):
        if inv_method.startswith("Stehfest"):
            P[i, :] = invert_stehfest_vec(Fvec, t, Nst)
        else:
            P[i, :] = invert_gaver_euler_vec(Fvec, t, M=M_gv, P=P_gv)

    Pwf = p0_psi - P

    # ================= Grupo A: presiones & caudales =================
    st.markdown("### Grupo A - Presiones y Caudales")

    # A1 - Pwf por pozo
    figA1 = go.Figure()
    for j in range(len(wells)):
        figA1.add_trace(go.Scatter(
            x=t_days, y=Pwf[:, j],
            mode="lines+markers",
            name=f"Pwf w{j+1}",
        ))
    figA1.update_layout(
        height=420,
        title="Presión por pozo",
        xaxis=dict(title="t [day]"),
        yaxis=dict(title="Pwf [psi]"),
        legend=dict(orientation="h", y=1.02),
    )
    apply_log_axis_controls(figA1, "A1", default_x_log=True)
    st.plotly_chart(figA1, use_container_width=True)

    # A2 - ΔP entre pozos vecinos
    if len(wells) >= 2:
        figA2 = go.Figure()
        for j in range(len(wells) - 1):
            dP = Pwf[:, j] - Pwf[:, j+1]
            figA2.add_trace(go.Scatter(
                x=t_days, y=dP,
                mode="lines+markers",
                name=f"ΔP w{j+1}-w{j+2}",
            ))
        figA2.update_layout(
            height=320,
            title="ΔP entre pozos vecinos",
            xaxis=dict(title="t [day]"),
            yaxis=dict(title="ΔP [psi]"),
        )
        apply_log_axis_controls(figA2, "A2", default_x_log=True)
    ...
    figA3.update_layout(
        height=320,
        title="Schedules de caudal q(t)",
        xaxis=dict(title="t [day]"),
        yaxis=dict(title="q [STB/d]"),
    )
    apply_log_axis_controls(figA3, "A3")
    st.plotly_chart(figA3, use_container_width=True)

    # A4 - Presión + caudal (doble eje)
    figA4 = go.Figure()
    for j in range(len(wells)):
        figA4.add_trace(go.Scatter(
            x=t_days, y=Pwf[:, j],
            mode="lines",
            name=f"Pwf w{j+1}",
        ))
    ...
    figA4.update_layout(
        height=420,
        title="Presión y caudal juntos",
        xaxis=dict(title="t [day]"),
        yaxis=dict(title="Pwf [psi]"),
        yaxis2=dict(title="q [STB/d]", overlaying="y", side="right"),
    )
    apply_log_axis_controls(figA4, "A4", default_x_log=True)
    st.plotly_chart(figA4, use_container_width=True)

    # ================= Grupo B: adimensionales =================
    st.markdown("---")
    st.markdown("### Grupo B - Adimensionales y Diagnósticos")
    ...
    figB1.update_layout(
        height=360,
        title="ΔP vs t (log-log)",
        xaxis=dict(title="t [day]"),
        yaxis=dict(title="ΔP [psi]", type="log"),
    )
    apply_log_axis_controls(figB1, "B1", default_x_log=True, default_y_log=True)
    ...
    apply_log_axis_controls(figB2, "B2", default_x_log=True)
    st.plotly_chart(figB2, use_container_width=True)

    # B3 - Q(t) pozo 1
    if schedules:
        w1 = list(schedules.keys())[0]
        tvals = [s["t_ini"] for s in schedules[w1]]
        qvals = [s["q"] for s in schedules[w1]]
        figB3 = go.Figure()
        figB3.add_trace(go.Scatter(
            x=tvals, y=qvals,
            mode="lines+markers",
            name="Q(t)",
        ))
        ...
        apply_log_axis_controls(figB3, "B3")
        st.plotly_chart(figB3, use_container_width=True)

    # B4 - pD + derivada (aprox)
    dp_dlogt = np.gradient(
        np.log(np.clip(P[:, 0], 1e-12, 1e12)),
        np.log(t_days),
    )
    figB4 = go.Figure()
    ...
    figB4.add_trace(go.Scatter(
        x=t_days, y=np.abs(dp_dlogt),
        mode="lines",
        name="|dpD/d ln t|",
    ))
    figB4.update_layout(
        height=360,
        title="pD y derivada (aprox.)",
        xaxis=dict(title="t [day]"),
        yaxis=dict(title="Magnitud"),
    )
    apply_log_axis_controls(figB4, "B4", default_x_log=True)
    st.plotly_chart(figB4, use_container_width=True)
...
with tabs[5]:
    ...
    if len(wells) < 2:
        st.warning("Se necesitan al menos 2 pozos para hacer sensibilidad.")
    else:
        w1 = wells[0]
        w2 = wells[1]

        xF_vals = np.linspace(max(1.0, w2.xF * 0.5), w2.xF * 1.8, 15)
        dP_list = []

        for xF_test in xF_vals:
            # copiar pozos, cambiando solo x_F del child
            w2_tmp = Well(
                x=w2.x, y=w2.y, nF=w2.nF,
                xF=xF_test,
                t_days=w2.t_days[:],
                q_stbd=w2.q_stbd[:],
            )
            wells_tmp = [w1, w2_tmp] + wells[2:]
            ...
            t_eval_s = t_ref_d * DAY_TO_S
            if inv_method.startswith("Stehfest"):
                pwf = invert_stehfest_vec(Ftmp, t_eval_s, Nst)
            else:
                pwf = invert_gaver_euler_vec(Ftmp, t_eval_s, M=M_gv, P=P_gv)

            P1 = p0_psi - pwf[0]
            P2 = p0_psi - pwf[1]
            dP_list.append(P1 - P2)

        dP_list = np.array(dP_list, float)
        ...
        figS.update_layout(
            height=420,
            title="Sensibilidad: ΔP(t*) vs x_F del child",
            xaxis=dict(title="x_F [ft]"),
            yaxis=dict(title="ΔP [psi]"),
        )
        apply_log_axis_controls(figS, "S")
...
with tabs[7]:
    ...
    apply_log_axis_controls(figK, "K")
